...
function maybeResumeFaceIntroPost() {
  ...
  if (typeof window.gisugoTakeFaceIntroDraft === 'function') {
    window.gisugoTakeFaceIntroDraft('post');
  }
}
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  const returningFromFaceIntro = entryParams.get('fvResume') === '1';
  const isEditOpen = !!(entryParams.get('edit') && entryParams.get('category'));
  if (!isEditOpen && !returningFromFaceIntro && typeof window.gisugoEnsureFaceIntro === 'function') {
    const faceReady = await window.gisugoEnsureFaceIntro('post', { stage: 'entry' });
    if (!faceReady) return;
  }
