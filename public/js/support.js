...
const PRODUCED_CUSTOMER_ALERT_TYPES = ['offer_accepted', 'application_received', 'application_milestone', 'gig_auto_paused', 'gig_review_needed', 'offer_rejected', 'worker_resigned', 'worker_feedback_received', 'worker_banned_gig_reopened'];
...
function getLocalizedAlertMessage(notif, type) {
    ...
    const localeMap = {
        english: {
            offer_sent: `You've been offered the gig ${gigTitleHtml}! Click to accept or decline.`,
        ...
        bisaya: {
            offer_sent: `Na-offeran ka sa gig ${gigTitleHtml}! I-click para modawat o modili.`,
        ...
        tagalog: {
            offer_sent: `May offer ka sa gig ${gigTitleHtml}! I-click para tanggapin o tanggihan.`,
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (typeof window.requireVerifiedEmailForPage === 'function') {
        const accessAllowed = await window.requireVerifiedEmailForPage({
            pageName: 'Support',
            redirectOnUnauth: `login.html?redirect=${encodeURIComponent(supportRedirectTarget)}`
        });
        if (!accessAllowed) {
            hideMessagesPageLoadingOverlay();
            return;
    ...
    // Hide role/lang chrome if present (Support is unified, no role tabs).
    document.querySelectorAll('.role-tabs, .messages-tabs, .alerts-lang-tabs').forEach((el) => {
    ...
    initializeMenu();
    if (typeof window.initializeSupportCompose === 'function') {
        window.initializeSupportCompose();
    }
...
function getSupportSubtopicCodeFromMessage(message) {
    ...
    const taxonomy = window.GISUGO_SUPPORT_TAXONOMY;
    const legacyMap = taxonomy && taxonomy.legacyContactTopicMap
        ? taxonomy.legacyContactTopicMap
        : {};
...
function getSupportEmptyStateHTML(messageText, detailText) {
    return `
        <div class="messages-placeholder">
            ...
            <h3>${messageText}</h3>
            <p>${detailText}</p>
            <small>Tap Write (✏️) to send a new request.</small>
        </div>
    `;
}
