...
- **Phase 9 — Ban product builds are done.** Ban + phone banlist are live. Not a new Ban
  ...
  + login UI and reject new `*@phone.gisugo.app` creates), after dummies are gone so you
  can still mint test accounts; (5) pre-launch phone audit. Permaban IP is **after
  launch**. Do not build a second signup rate limiter.
- **Phase 10 — retired as an open phase.** In-app Support stays **on** (it works). The
...
- [x] **#8 Architecture + cost study — COMPLETE (2026-07-27).** Full detail in
      ...
      **Resolved:** Gig Moderation (Posted = glance, Reported/Suspended = queues, search by title
      ...
      counters incl. Age Groups off `dateOfBirth` (required under About Me for 18+; Education stays optional) + Regional Distribution off a
...
- [ ] **Phase 9: Permanently Ban User — DECIDED (2026-08-10): Disable Auth login,
      ...
      **Microtasklist**
      ...
      4. **[x] Phone banlist + uniqueness write.** `saveUserPhone` callable.
         ...
         leaves the list entry (that uid only). **Live 2026-08-25.**
...
- [x] **Phase 10: Support thread engine — Chapters 1–4 LIVE (2026-08-14).
      Retired as an open phase 2026-08-28.** Owner: in-app Support works; leave it
      on. Chapters 5–7 (Email / WhatsApp shelf) stay below as **reference only** —
      not an agent task, not next, not a launch gate. Do **not** hide Reply. Do
      **not** start 5–7 unless the owner reopens this on purpose.
      New phase (Phase 4 stays closed). Owner confirmed the test loop (text + photo both
      ways, Mark Resolved, hourglass). **Leave the in-app desk on.**
      ...
      **Microtasklist — Chapters 1–4 done and live. Chapters 5–7 are reference only
      (retired 2026-08-28). Do not start them unless the owner reopens Phase 10.**
...
- [x] **Phase 12: Applications + notifications lockdown (Track B). SHIPPED 2026-09-06/10.**
      ...
      **Microtasklist**
      ...
      3. **[x] Server: accept rejects the others.** `workerAcceptRejectOthers`
         ...
         **Step 2 review (2026-08-30, no extra fix):** Allowlist matches
         A1–A3, A5–A12. Buttons still call `createNotification`. Who-may-send
         and the three shapes match today’s browser behavior. Smoke already
         ran (deny + `dryRun`). **Not a miss:** A6 slots-reopened is
         owner-only on `createUserAlert` on purpose (owner reject). The
         Accept sweep cannot have the worker call that function for other
         people — Step 3 writes those cards **inside** the sweep (reuse
         ...
         **Step 3 microtasklist (do in order; audit + smoke each; stop
         ...
         1. **[x] Lock the contract (no code).** One callable
            ...
            (client may have already flipped the gig). Sweep-only: do
...
- [x] **~~KNOWN LIMITATION~~ → SOLVED via Facebook DEVICE LOGIN (2026-07-14, deployed).**
      ...
      **2026-09-18:** Tried leading phone Facebook with this app-confirm flow. **Failed
      smoke** (code copy/paste, tab does not resume signup after approve — same reasons
      it was demoted in July). **Reverted same day** to website redirect first; device
      login stays rescue / iOS ≤15 only. A website cannot do seamless Facebook-app SSO.
      Native GISUGO is the real app-to-app path.
      ...
      Device-class matrix (restored 2026-09-18): iPhone 7/old-iOS → device rescue;
      iOS 16+ / Android → website redirect first; tokenless hop still auto-rescues;
      desktop → website redirect. Seamless Continue-as only if that browser already
      has a facebook.com session.
